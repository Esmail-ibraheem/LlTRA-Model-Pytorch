...
class FeedForwardBlock(nn.Module):
    ...
    def forward(self, x):
        return self.Linear_1(self.dropout(torch.relu(self.Linear_2(x))))

class MultiHeadAttnetionBlock(nn.Module):
    ...
    def __init__(self, d_model: int, heads: int, dropout: float) -> None:
        ...
        assert d_model % heads == 0 , "d_model is not devisbal by heads"
    ...
    def forward(self, query, key, value, mask):
        ...
        x, self.self_attention_scores = MultiHeadAttnetionBlock.Attention(Query, Key, Value, mask, self.dropout)
...
class EncoderBlock(nn.Module):
    ...
    def __init__(self, encoder_self_attention_block: MultiHeadAttnetionBlock, encoder_feed_forward_block: FeedForwardBlock, dropout: float) -> None:
        super().__init__()
...
class DecoderBlock(nn.Module):
    ...
    def __init__(self, decoder_self_attention_block: MultiHeadAttnetionBlock, decoder_cross_attention_block: MultiHeadAttnetionBlock, decoder_feed_forward_block: FeedForwardBlock, dropout: float) -> None:
        super().__init__()
...
def TransformerModel(source_vocab_size: int, target_vocab_size: int, source_sequence_length: int, target_sequence_length: int, d_model: int = 512, Layers: int = 6, heads: int = 8, dropout: float = 0.1, d_ff: int = 2048)->TransformerBlock:
    ...
    for _ in range(Layers):
        encoder_self_attention_block = MultiHeadAttnetionBlock(d_model, heads, dropout)
    ...
    DecoderBlocks = []
    for _ in range(Layers):
        decoder_self_attention_block = MultiHeadAttnetionBlock(d_model, heads, dropout)
        decoder_cross_attention_block = MultiHeadAttnetionBlock(d_model, heads, dropout)
