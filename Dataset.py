...
class BilingualDataset(Dataset):
    ...
    def __getitem__(self, index) -> any:
        ...
        encode_source_tokenizers = self.source_tokenizer.encode(source_text).ids 
        encode_target_tokenizers = self.target_tokenizer.encode(target_text).ids 

        encode_source_padding = self.sequence_length - len(encode_source_tokenizers) - 2
        encode_target_padding = self.sequence_length - len(encode_target_tokenizers) - 1
        ...
        encoder_source = torch.cat(
            [
                self.SOS_token,
                torch.tensor(encode_source_tokenizers, dtype=torch.int64),
                self.EOS_token,
        ...
        encoder_target = torch.cat(
            [
                self.SOS_token,
                torch.tensor(encode_target_tokenizers, dtype= torch.int64),
